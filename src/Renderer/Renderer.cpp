...
QOpenGLBuffer Renderer::s_fullscreenPosAndUV_VBO = QOpenGLBuffer(QOpenGLBuffer::VertexBuffer);
...
void Renderer::SetupShaderForFullScreen(QOpenGLShaderProgram& shader) {
    s_fullscreenPosAndUV_VAO.bind();
    //
    ...
    shader.setAttributeArray(VERTEX_ATTR_POS, GL_FLOAT, (void*) 0,                  2, 4 * sizeof(float));
    shader.setAttributeArray(VERTEX_ATTR_UV, GL_FLOAT,  (void*)(2 * sizeof(float)), 2, 4 * sizeof(float));
    //
    s_fullscreenPosAndUV_VAO.release();
}

void Renderer::DrawFullScreen(QOpenGLShaderProgram& shader) {
    s_fullscreenPosAndUV_VAO.bind();
    shader.bind();
